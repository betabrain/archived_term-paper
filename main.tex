...
more random, e.g. \citeA{geurts2006extremely}, \citeA{liu2005maximizing},
...
optimality criterion, a naive stop criterion only capabable of handling
perfecly partitionable data, and lacks variable importance measures.
...
For each node the algorithm then progesses to check whether the node should be split,
...
Since Clojure features immutable data structures this increases memory usage only negligably.
...
The following \texttt{rf-train} function is the top-level function resposible for
...
\newpage
\section{Appendix}
The following Clojure source code is a complete example implementation
of random forests.
Parts of it are explained earlier in this paper.
To run the examples, install the \texttt{Leiningen} build system
\cite{leiningen}, save the following source code the indicated files,
