...
(defn extend-node [{:keys [dataset oob] :as node} mtry]
  (if (terminal? dataset)
    ...
    (let [[s l r] (best-split dataset mtry)]
      (merge node {:criterion s :left  {:dataset l :oob r}
                                :right {:dataset r :oob l}}))))
